...
def read_webhouse_key():
    ...
    Return either None (no key found), or a string representing the key.
    Remeber: put search.key in your .gitignore file to avoid commiting it!
    """
...
def run_query(search_term, size=10):
    ...
    return (default of 10), returns a list of results from Webhose API,
    with each result consisting of a title, link and summary.
    """
    Webhose_api_key = read_webhouse_key()
    ...
    # search_url is a string split over multipe lines.
...
def raw_input():
    query_term = input("Enter your query: ")
    results = run_query(query_term)
    print(results)
